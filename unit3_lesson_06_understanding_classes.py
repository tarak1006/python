...
def test_classes_bound_and_unbound_methods():
    ...
    #assert None == q1_push.__self__      #bound method, is associated with an instance
...
def test_instances_state():
    ...
    try:
        age = r1.age
        assert False, "Should not reach here"
    except AttributeError as ae_: #what exception do you get?
        assert True
...
class Person():
    ...
    def get_bmi_result(self):
        bmi=float(self.__weight)/(float(self.__height)*float(self.__height)*(0.3048*0.3048))
        if bmi<18.5:
            return "underweight"
