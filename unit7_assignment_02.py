...
def profile_timeit():
    actuals=[]
    funcn_list = [numbers_string1, numbers_string2, numbers_string3, num_strings4]
    for i in funcn_list:
        for j in [10,100,1000,10000]:
            actuals=timeit.repeat(setup='i.funcn_name',repeat=5,number=1)
        print i.func_name + ", count = " + str(j) + ", min = " + str(min(actuals)) + ", actuals = " + str(actuals)
