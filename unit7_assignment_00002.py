...
def profile_timeit():
    actuals=[]
    func_list = [unit7_conversion_methods.numbers_string1,unit7_conversion_methods.numbers_string2,unit7_conversion_methods.numbers_string3,unit7_conversion_methods.num_strings4]
    for func in func_list:
        for count in [10, 100, 1000, 10000]:
            actuals = timeit.repeat(setup ='from unit7_conversion_methods import numbers_string1,numbers_string2,numbers_string3,num_strings4',stmt='num_strings4(j)',repeat=5,number=1)
            print func.func_name + " ,  count = " + str(count) + ", min = " + str(min(actuals)) + ", actuals = " + str(actuals)
