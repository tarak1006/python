...
def get_lower_to_upper_dict():
    ...
    """
    return dict(zip(string.lowercase,string.ascii_uppercase))
...
def get_encoding_dict():
    ...
    """
    return dict(dict(zip(string.uppercase,range(1,27))),**(dict(zip(string.lowercase,range(1,27)))))
...
def bit_count_even(number):
    ...
    """
    return bool((bin(number).count("1"))%2==0)
